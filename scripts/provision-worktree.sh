...
init_args=(worktree init --force --name "$worktree_name" --instance "$worktree_instance_id")
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]] && existing_worktree_config_is_usable; then
  ...
  if [[ "${PAPERCLIP_SKIP_WORKTREE_CLI_INIT:-false}" != "true" ]] && paperclipai_command_available; then
